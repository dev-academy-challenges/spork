...
const template = (w) =>
  w.schedule(
    ...
    w.on(w.week(1), w.fri(), w.all(), w.deploy('kata-typescript')),
    w.on(w.week(2), w.mon(), w.all(), w.deploy('react-paws-for-effect', 'charlottes-web-log')),
    w.on(w.week(2), w.tue(), w.all(), w.deploy('broken-kaleidoscope', 'enspiraled')),
    w.on(w.week(2), w.wed(), w.all(), w.deploy('worldwide-routing')),
    w.on(w.week(2), w.thu(), w.all(), w.deploy('react-form-demo', 'boilerplate')),
    w.on(w.week(2), w.fri(), w.except(w.akl()), w.deploy('lightning-talks')),
    w.on(w.week(3), w.mon(), w.all(), w.deploy('consuming-external-apis')),
    w.on(w.week(3), w.tue(), w.all(), w.deploy('query-em-al')),
    w.on(
      w.week(3),
      w.wed(),
      w.all(),
    ...
    w.on(w.week(3), w.thu(), w.all(), w.deploy('boilerplate')),
    w.on(
      w.week(4),
      w.mon(),
      w.all(),
      w.deploy('express-server', 'serverside-rendering')
    ),
    w.on(
      w.week(4),
      w.tue(),
    ...
    w.on(w.week(4), w.thu(), w.all(), w.deploy('boilerplate')),
